...
-- Tiempo de generación: 03-02-2026 a las 15:26:09
...
ALTER TABLE `ads` 
    ADD COLUMN `ad_type` ENUM('image','adsense') NOT NULL DEFAULT 'image' AFTER `status`,
    ADD COLUMN `ad_code` TEXT NULL AFTER `ad_type`;

ALTER TABLE `ads` 
    ADD COLUMN `zone` VARCHAR(50) NULL DEFAULT NULL AFTER `ad_code`;

    ...
    <div class="container py-3">
        ...
        <div class="row g-4">
            ...
            <div class="col-md-6 col-lg-4">
                <a href="<?= URLBASE ?>/programas/<?= htmlspecialchars($prog['slug']) ?>/" class="text-decoration-none text-dark">
                    <div class="card h-100 shadow-sm" style="border-radius:8px; overflow:hidden; transition:transform .2s;"
                        ...
                        <div class="card-body">
                            <?php if (!empty($prog['category'])): ?>
                                <span class="badge bg-primary mb-2"><?= htmlspecialchars($prog['category']) ?></span>
                            <?php endif; ?>
                            <h5 class="card-title"><?= htmlspecialchars($prog['title']) ?></h5>
                            <?php if (!empty($prog['hosts'])): ?>
                                <small class="text-muted"><i class="fas fa-user me-1"></i><?= htmlspecialchars($prog['hosts']) ?></small>
                            <?php endif; ?>
                        </div>
                    </div>
                </a>

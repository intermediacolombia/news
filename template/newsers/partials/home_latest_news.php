                ...
                <div class="bg-light rounded">
                    <div class="rounded-top overflow-hidden">
                        <a href="<?= URLBASE ?>/<?= htmlspecialchars($news['category_slug']) ?>/<?= htmlspecialchars($news['slug']) ?>/">
                            <img src="<?= img_url($news['image']) ?>" class="img-zoomin img-fluid rounded-top w-100" alt="<?= htmlspecialchars($news['title']) ?>">
                        </a>
                    </div>
                    <div class="d-flex flex-column p-4">
                        <a href="<?= URLBASE ?>/<?= htmlspecialchars($news['category_slug']) ?>/<?= htmlspecialchars($news['slug']) ?>/" class="h4">
                            <?= htmlspecialchars($news['title']) ?>
                        </a>
                        <div class="d-flex justify-content-between">
                            <?php if (!empty($news['author'])): ?>
                            <span class="small text-body">by <?= htmlspecialchars($news['author']) ?></span>
                            <?php else: ?>

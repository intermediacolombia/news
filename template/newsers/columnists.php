...
if (!empty($usuario['foto_perfil'])) {
    ...
    $fotoPerfil = 'data:image/svg+xml;base64,' . base64_encode("<svg width='200' height='200' xmlns='http://www.w3.org/2000/svg'><rect width='200' height='200' fill='#007bff'/><text x='50%' y='50%' font-size='60' fill='white' text-anchor='middle' dy='.35em' font-family='Arial'>{$iniciales}</text></svg>");
}
...
<div class="container-fluid py-5">
    <div class="container py-3">
        <div class="bg-light rounded p-4 text-center mb-4">
            ...
            <p class="text-muted text-uppercase small letter-spacing-1">Columnista</p>
            <span class="badge bg-primary"><?= count($posts) ?> columna<?= count($posts) !== 1 ? 's' : '' ?> publicada<?= count($posts) !== 1 ? 's' : '' ?></span>
        </div>

        <h3 class="mb-4">Columnas de opinión</h3>
                ...
                <div class="card h-100 shadow-sm">
                    ...
                    <div class="card-body">
                        <?php if (!empty($post['category_name'])): ?>
                            <span class="badge bg-secondary mb-2"><?= htmlspecialchars($post['category_name']) ?></span>
                        <?php endif; ?>
                        <h5 class="card-title">
                            <a href="<?= $postUrl ?>" class="text-decoration-none text-dark"><?= htmlspecialchars($post['title']) ?></a>
                        </h5>
                        <p class="card-text text-muted small"><?= truncate_text($post['seo_description'] ?: $post['content'], 100) ?></p>
                        <small class="text-muted"><i class="far fa-calendar me-1"></i><?= date('d M, Y', strtotime($post['created_at'])) ?></small>
                    </div>
                </div>
            </div>
            <?php endforeach; ?>
